...
const double ticksPerwheelRev = 570000; // 4*500*285 //not in use. just a reference for now
const double wheelRadius = 0.124; // 55.18;
const double WHEEL_BASE = 0.600; //223.8375mm actually
const double TICKS_PER_M = 731970; //1.1645; //1.365 is on hard floor. carpet avg is 1.1926. overall avg = 1.1645
...
void setInitialPose(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& rvizClick)
{
     ROS_INFO_STREAM("Info: i am in set initialpose");
...
void Calc_Left(const std_msgs::Int64& lCount)
{  // ROS_INFO_STREAM("left wheel counts subscribed");
...
void Calc_Right(const std_msgs::Int64& rCount)
{
    //ROS_INFO_STREAM("right wheel counts subscribed");
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber subInitialPose = node.subscribe("initial_2d", 1000, setInitialPose);//,ros::TransportHints().tcpNoDelay());
    ...
    ROS_INFO_STREAM("Info: Odom node started");
    ros::Rate loop_rate(30);
    while(ros::ok())
    {	ros::spinOnce();
        //ROS_INFO_STREAM("initialPoseRecieved="<<initialPoseRecieved);
        if(initialPoseRecieved)
        {   //ros::spinOnce();
            ROS_INFO_STREAM("Info: initial pose received");
            update_odom();
            publish_quat();
