...
class DeadReckoningOdom():
    ...
    def __init__(self):
        ...
        self.nodeName='Odomnode'
    
        # these are the topic names
        # these topic names have to match the topic names specified in the Arduino code
        self.topicNameLeftEncoder='leftWheel'
        self.topicNameRightEncoder='rightWheel'
        
        #frame names for broadcasting odometry 
        self.baseFrameName='base_link' #rospy.get_param('~base_frame_id','base_link')
        self.odomFrameName= 'odom' #rospy.get_param('~odom_frame_id','odom')
        
        # wheel radius in meters
        self.wheelRadius=0.124 
        # distance between the wheels in meters 
        self.distanceWheels=0.600
        
        # encoder LM393 specs 
        # this encoder will read 20 pulses per one full revolution 
        self.encoderPulsesConstant=162000
        ...
        rospy.Subscriber(self.topicNameLeftEncoder, Int64, self.callBackFunctionLeftEncoder)
        # subscribe to the messages from the right encoder
        rospy.Subscriber(self.topicNameRightEncoder, Int64, self.callBackFunctionRightEncoder)
